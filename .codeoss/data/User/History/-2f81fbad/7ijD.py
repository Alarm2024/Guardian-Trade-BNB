...
def beep(duration, freq=800, vol=0.3):
    # AudioClip frame function should handle numpy arrays of time 't' and return (N, 1) for mono
    make_frame = lambda t: (vol * np.sin(2 * np.pi * freq * np.array(t))).reshape(-1, 1)
...
for i, line in enumerate(lines):
    ...
    t = t.with_start(i*5).with_duration(4).with_effects([mp.video.fx.CrossFadeIn(0.5)])
    clips.append(t)
...
industry = ColorClip(size=(SAFE_W, SAFE_H), color=(40,40,50), duration=50)
...
final_bg = ColorClip(size=(W,H), color=(0,0,0), duration=15).with_effects([mp.video.fx.CrossFadeIn(0.5)])
headline2 = make_safe_text("50 Million Tonnes Mitigated.\nSerious impact.", 80)
final = CompositeVideoClip([final_bg, headline2])
swell = beep(15, 250, 0.2)
swell = swell.fx(audio_fadeout, 3.0)
final = final.with_audio(swell)
