                    ...
                    for update in data["result"]:
                        ...
                        if "message" in update and "text" in update["message"]:
                            ...
                            if text == "/status":
                                ...
                                if not self.ai_active:
                                    self.send_telegram("❌ AI not configured.")
                                    continue
                                try:
                                    m_list = [m.name for m in self.client.models.list() if 'generateContent' in m.supported_actions]
                                    self.send_telegram("📋 *Available Models:*\n`" + "`\n`".join(m_list[:10]) + "`")
                                except Exception as e:
                                    self.send_telegram(f"❌ Error listing models: `{e}`")
                            elif text == "/payout":
