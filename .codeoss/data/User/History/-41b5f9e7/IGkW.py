...
class CarbonMiner:
    """
    Guardian Angel v3.7 - Stability & Performance Fix
    """
    def __init__(self):
        ...
        if self.gemini_key and self.gemini_key != "your_gemini_api_key_here":
            try:
                ...
                # Force 1.5-flash for maximum free-tier reliability
                selected_model = 'gemini-1.5-flash'
                self.model = genai.GenerativeModel(selected_model)
                self.ai_active = True
                print(f"[✔] AI Engine Forced: {selected_model}", flush=True)
        ...
        for i in range(retries):
            try:
                ...
                if "429" in err_msg or "quota" in err_msg:
                    # Hard quota limit hit. Set cooldown for 5 minutes.
    ...
    def fetch_telemetry(self):
        ...
        return {"location": zone, "mitigation_tonnes": round(reduction, 2), "baseline": round(baseline, 2)}

    def verify_mitigation_ai(self, data):
        prompt = (f"As a Carbon Credit Auditor, evaluate this telemetry: "
                  f"Location: {data['location']}, "
                  f"Baseline: {data['baseline']} tonnes, "
                  f"Mitigation: {data['mitigation_tonnes']} tonnes. "
                  f"Is this mitigation significant and valid? Reply with YES or NO and a brief reason.")
        res = self.call_gemini(prompt)
        if res: return "YES" in res.upper()
    ...
    def handle_ai_chat(self, user_msg):
        ...
        if res:
            ...
            self.send_telegram("⚠️ *AI Quota Hit:* I'm resting for a moment to avoid limits. Try again in a minute!")
    ...
    def get_telegram_updates(self):
        ...
        while True:
            try:
                ...
                print(f"[.] Polling Telegram... (Last ID: {last_id})")
                resp = requests.get(url, params=params, timeout=25)
                
                if resp.status_code == 409:
                    print("[!] Telegram Conflict (409): Another instance is likely running. Please stop other bot processes.")
                    time.sleep(10)
                    continue
