...
class CarbonMiner:
    ...
    def send_document(self, file_path):
        ...
        try:
            with open(file_path, 'rb') as f:
                files = {'document': f}
                r = requests.post(url, data={'chat_id': self.chat_id}, files=files, timeout=20)
                if r.status_code != 200:
                    print(f"[!] Telegram sendDocument Error ({r.status_code}): {r.text}", flush=True)
        except Exception as e:
            print(f"[!] Telegram File Send Failed: {e}", flush=True)
    ...
    def call_gemini(self, prompt, retries=3):
        ...
        for i in range(retries):
            try:
                # Standard model identifier for the google-genai Client SDK
    ...
    def export_report(self):
        ...
        self.send_telegram(f"✅ *Manifest Generated:* `{len(self.certificates)}` assets prepared. Sending report...")
        self.send_document(manifest_path)
